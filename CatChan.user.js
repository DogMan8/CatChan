...
//      show_tooltip : true,
      tooltip: {show: true, popup_delay:4000, popdown_delay:1000},
      catalog: {
        indexing: 0,
//        max_threads : 512,
...
//    var tooltip_on = false;
    return {
      apply_prep: function(pn,set,propagate){
...
//        }
      ...
      tooltips: (function(){
        ...
        tooltip.style.fontWeight = 'normal';
        var tooltip_on = null;
        var body_0 = document.getElementsByTagName('body')[0];
        var func = {};
        var show_timer = null;
        var hide_timer = null;
        var str_tooltip = '';
        ...
        function add_hier(pn){
          ...
          if (pref.test_mode['5']) for (var i=0;i<all.length;i++) if (!str[all[i].name]) str[all[i].name] = 'Name: ' + all[i].name;
...
//          if (!tooltip_on) body_0.appendChild(tooltip);
//          else {
//            if (hide_timer!==null) {clearTimeout(hide_timer);hide_timer=null;}
...
//        }
        ...
        function hide_exe(){
          if (tooltip_on) {
            if (show_timer===null) body_0.removeChild(tooltip);
...
//          '&emsp;Max threads in catalog: <input type="text" name="catalog.max_threads" size="4" style="text-align: right;"><br>',
          ...
          '&emsp;&emsp;&emsp;<input type="radio" name="catalog_open_last50" value="no">No. Open link of entire thread<br>'+
          '&emsp;&emsp;&emsp;<input type="radio" name="catalog_open_last50" value="exist">If it exists<br>'+
          '&emsp;&emsp;&emsp;<input type="radio" name="catalog_open_last50" value="exist_watch">If it exists and unread posts are less than 50<br>'+
          '&emsp;&emsp;&emsp;<input type="radio" name="catalog_open_last50" value="speculative">If it is estimated to exist<br>'+
          '&emsp;&emsp;&emsp;<input type="radio" name="catalog_open_last50" value="spec_watch">If it is estimated to exist and unread posts are less than 50<br>'+
...
//////          '&emsp;&emsp;&emsp;<input type="radio" name="catalog.order.sticky" value="hide"> hide<br>'+
          ...
          '&emsp;&emsp;<input type="checkbox" name="catalog_footer"> Show information footer<br>'+
          '&emsp;&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_br"> Always over/under the image<br>'+
          '&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_show_board_name"> Show board\'s name(board)<br>'+
          '&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_show_nof_rep"> Show num of new replies(nr)<br>'+
          '&emsp;&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_show_nof_rep_to_me"> Show num of new replies to me(nrtm)<br>'+
          '&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_show_flag"> Show recent flags(flags)<br>'+
          '&emsp;&emsp;&emsp;<input type="checkbox" name="catalog_footer_show_page"> Show page No.(page)<br>'+
          '&emsp;&emsp;&emsp;Design:<br>'+
          '&emsp;&emsp;&emsp;&emsp;<input type="radio" name="catalog_footer_design" value="native"> U: nrtm/nr / R: r / I: i / P: page /board/ flags<br>'+
          '&emsp;&emsp;&emsp;&emsp;<input type="radio" name="catalog_footer_design" value="condensed"> nrtm/nr/r/i/page /board/ flags<br>'+
...
//          '5',
          ...
          '&emsp;delay to popup: <input type="text" name="tooltip.popup_delay" size="6" style="text-align: right;"> ms<br>'+
          '&emsp;delay to popdown: <input type="text" name="tooltip.popdown_delay" size="6" style="text-align: right;"> ms<br>',
          'CatChan<br>'+
          'Version 2015.03.28.0<br>'+
...
//      }
//    },
    parse_funcs_on_demand: { // will be changed to dynamic function.
      get no() {return this.exe_sub_on_demand('no');},
      get ths() {return this.exe_sub_on_demand('ths');},
      get key() {return this.exe_sub_on_demand('key');},
      get time_bumped() {return this.exe_sub_on_demand('time_bumped');},
      get nof_posts() {return this.exe_sub_on_demand('nof_posts');},
      get nof_files() {return this.exe_sub_on_demand('nof_files');},
      get time_created() {return this.exe_sub_on_demand('time_created');},
      get posts() {return this.exe_sub_on_demand('posts');},
      get sub() {return this.exe_sub_on_demand('sub');},
      get name() {return this.exe_sub_on_demand('name');},
      get com() {return this.exe_sub_on_demand('com');},
      get flag() {return this.exe_sub_on_demand('flag');},
      get flags() {return this.exe_sub_on_demand('flags');},
      get footer() {return this.exe_sub_on_demand('footer');},
      get sticky() {return this.exe_sub_on_demand('sticky');},
      get format() {return this.exe_sub_on_demand('format');},
      get pn() {return this.exe_sub_on_demand('pn');},
      get time() {return this.exe_sub_on_demand('time');},
      get time_posted() {return this.exe_sub_on_demand('time_posted');},
//      get html_org() {return this.exe_sub_on_demand('html_org');},

      exe_sub_on_demand : function(prop){
//console.log(prop);
        Object.defineProperty(this,prop,{value:this.parse_funcs[prop](this), enumerable:true, configurable:true, writable:true});
        return this[prop];
      }
    },
  };
  site2['8chan'] = {
    ...
    check_func : function(){
      if (window.location.href.search(/8chan.co|8ch.net/)!=-1) { // 8chan
        ...
        site.whereami = (document.getElementsByTagName('header')[0].innerHTML.indexOf('404 Not Found')!=-1)? '404'
                      : (window.location.href.search(/catalog\.html/)!=-1)? 'catalog'
                      : (window.location.href.search(/res\.html/)!=-1)? 'thread'
                      : 'page';
        return true;
      } else {
        if (!brwsr.ff) {
    ...
    embed_to: {
      'page_top'     : document.getElementsByName('postcontrols')[0],
      'page_bottom'  : document.getElementsByClassName('boardlist bottom')[0],
      'thread_top'   : document.getElementsByName('postcontrols')[0],
      'thread_bottom': document.getElementsByName('postcontrols')[0],
    },
...
//      for (var i=0;i<posts.length;i++) {
      ...
      while (i>=0) {
        ...
        var time = Date.parse(posts[i].getElementsByTagName('time')[0].getAttribute('datetime')) - pref.localtime_offset*3600000;
        obj[i] = {
          time: time,
...
//            site4.parse_funcs.call(this,ths[ths.length-1],req2);
//          }
//          return ths;
//        },
...
if (!pref.test_mode['0']) {
          ...
          for (var i=0;i<mixs.length;i++) if (mixs[i].tagName==='DIV')
            ths[ths.length] = {pn:mixs[i], page:Math.floor(i/15)+'.'+i%15, domain:doc.domain, board:doc.board, parse_funcs:doc.parse_funcs, __proto__:site4.parse_funcs_on_demand};
}
          return ths;
...
//}
          var pn = site2['8chan'].catalog_json2html3(th,th.board);
...
//        sticky: function(th){return (th.sticky===1);}, // prevent memory leak.
        ...
        sticky: function(th){return (th.hasOwnProperty('sticky'))? th.sticky : false;},
        proto: 'catalog_html',
      },
      'page_html' : {
        before_test : ['ths',':ITER',':ALL','ths',['key','time_bumped','nof_posts','nof_files']],
...
//        ths: function(doc) {
//          var pc = (doc.pn)? doc.pn.getElementsByName('postcontrols')[0] : doc.getElementsByName('postcontrols')[0]; // fusion with catalog_threads_in_page
//          var th = [];
//          if (pc)
//            for (var i=0;i<pc.childNodes.length;i++)
//              if (pc.childNodes[i].id && pc.childNodes[i].id.substr(0,6)=='thread') {
//                pc.childNodes[i].removeAttribute('class');
//                site2['8chan'].absolute_link(pc.childNodes[i]);
//                th.push((doc.pn)? {pn:pc.childNodes[i], page:doc.page + '.' + th.length} : pc.childNodes[i]);
//              }
//          return th;
//        },
        ths: function(doc) {
          var pc = doc.pn.getElementsByName('postcontrols')[0];
          var th = [];
          if (pc)
            for (var i=0;i<pc.childNodes.length;i++)
              if (pc.childNodes[i].id && pc.childNodes[i].id.substr(0,6)=='thread') {
                ...
                site2['8chan'].absolute_link(pc.childNodes[i]);
if (!pref.test_mode['0']) {
                th[th.length] = {pn:pc.childNodes[i], page:doc.page + '.' + th.length};
} else { 
                th[th.length] = {pn:pc.childNodes[i], page:doc.page + '.' + th.length,
                  domain:doc.domain, board:doc.board, parse_funcs:doc.parse_funcs, __proto__:site4.parse_funcs_on_demand};
}
              }
          return th;
        },
...
if (!pref.test_mode['0']) {
        ...
        sub:  function(post){return post.pn.getElementsByClassName('subject')[brwsr.innerText];},
        name: function(post){return post.pn.getElementsByClassName('name')[brwsr.innerText];},
        com:  function(post){return post.pn.getElementsByClassName('body')[brwsr.innerText];},
...
//  }
  for (var i in site2) if (i!=='DEFAULT' && i!=='common') {
    site2[i].__proto__ = site2['DEFAULT'];
    for (var j in site2[i].parse_funcs) {
      var own_prop = site2[i].parse_funcs[j].hasOwnProperty('proto');
      site2[i].parse_funcs[j].__proto__ = (own_prop)? site2[i].parse_funcs[site2[i].parse_funcs[j].proto]
                                                    : site4.parse_funcs;
      if (own_prop) delete 'proto';
//      if (j.substr(-5,5)==='_json' && j.replace(/_json$/,'_html') in site2[i].parse_funcs) site2[i].parse_funcs[j].__proto__ = site2[i].parse_funcs[j.replace(/_json$/,'_html')];
//      else if (j.substr(0,6)==='thread' && j.replace(/^thread/,'page') in site2[i].parse_funcs) site2[i].parse_funcs[j].__proto__ = site2[i].parse_funcs[j.replace(/^thread/,'page')];
//      else site2[i].parse_funcs[j].__proto__ = site2.common.parse_funcs;
    }
  }
...
//      callback(key, value, callback_arg);
      ...
      if (!from_cache && data_type==='html') {
        ...
        value.response.getElementsByTagName('head')[0].innerHTML = '';
      }
  ...
  var cnst = (function(){
    ...
    function maximize(pn,tb,req,state,callback_func){
      ...
      if (req==3) { // maximize
        ...
        var ref = site2[site.nickname].embed_to[site.whereami+((req==0)?'_top':'_bottom')];
        if (ref){
...
//          pn.style.border = 'none';
        ...
        while (i<funcs.length) {
          ...
          if (arg=='tb') {
            ...
            pn.innerHTML = '<div>' +
              '<div style="float: left"><button type="button">-</button><button type="button"><</button><button type="button">></button>&emsp;</div>' +
...
////        pn.style.background = '#b5ccf9';
////        pn.innerHTML = '<div>' +
////          '<div style="float: left"><button name="rollup">-</button><button name="opacity,-0.1"><</button><button name="opacity,0.1">></button>&emsp;</div>' +
...
////        } else {
////          var ref = site2[site.nickname].embed_to[site.whereami+((name==='embed_top')?'_top':'_bottom')];
////          if (ref){
...
//    var obj = site2[site.nickname].get_posts2(document,dummy);
    ...
    threads[name][19] = [0,0,0,0,null,false];
...
//                                        : '');
    ...
    function triage_event(){
      ...
      triage_exe(triage_str[i][j],triage_str[i][j+2]);
    }
    function triage_exe(com0,com1){
      send_message('parent',[['TRIAGE',[name,com0,com1,true,time_lastpost + pref.localtime_offset*3600000]]]); // patch
...
//      base_thread.removeEventListener('DOMSubtreeModified',updated_buf,false);
  ...
  function thread_reader_init(){
    if (common_obj.thread_reader===null && site.features.thread_reader && site.isthread && pref.thread_reader.use) common_obj.thread_reader = make_thread_reader();
    else if (!pref.thread_reader.use && common_obj.thread_reader) common_obj.thread_reader = common_obj.thread_reader();
  }
...
//            '<textarea style="height:1em" cols="25" name="catalog.filter.kwd_str"></textarea>'+
            '<input type="text" name="catalog.filter.kwd_str" size="25">'+
...
//      site2[site.nickname].prep_own_posts();
      ...
      var scan_boards = (function(){
        ...
        function scan_boards_keyword_callback(key,value,scan_key){
          if (value.status==200) {
            site3[site.nickname].boards = ('response' in value)? value.response : JSON.parse(value.responseText);
...
//          while (obj[sb.max-1].max===null) sb.max--;
  ...
  //console.log('spawn '+sb.crawler);
        ...
        function scan_boards_keyword(args,status){
          var sender = args[0];
          var sb = args[1];
          if (sb.idx<sb.max && sb.found_threads<sb.max_threads && status<500) {
            ...
            if (--sb.crawler==0) {
              scan_progress.innerHTML = sb.found_threads+'/'+sb.scanned+', '+sb.found_boards+'/'+sb.max
                                      ...
                                      +( (sb.error!=='')? ', Error at loading '+sb.error :
                                         ((sb.found_threads>=sb.max_threads)? ', Aborted.(Reached upper limit)' : ''))
                                      + '</span>'
//              sb.max = sb.idx;
...
//var time_0 = performance.now();
                ...
                if (threads[name][19][0]>threads[name][8][4]) threads[name][19][0] = -1;
...
//              }
  ...
  //        return pn.tags;
      ...
      pn_filter.getElementsByTagName('*')['catalog.filter.kwd_str'].onkeyup = pn_filter_changed;
...
//      var triage_parent = (pref.catalog.embed && site.catalog)? site2[site.nickname].catalog_get_native_area() : pn12_1;
      if (pref.catalog.maximize_at_initial) pn12_0.childNodes[1].childNodes[3].onclick();
      ...
      function scan_tags_init(str2, reload){
        ...
        pn_tag_list.onchange = function(){prep_tag_str(null, reload, true);};
...
//      pn12_1.style.height = '';
      ...
      function remake_triage(){
        catalog_triage_out();
        if (pref.tooltip.show) pref_func.tooltips.remove(pn12_triage);
        make_triage_entry();
      }
      function make_triage_entry(){
        var pn12_triage_all = common_func.make_triage({onclick:triage_event, wheelpatch:(!(pref.catalog.embed && site.catalog))? triage_wheel : null});
...
//          insert_thread_from_native(ths[i], site.nickname, site.board, false, date);
          insert_thread_passed_test(ths[i], 'catalog_html', date);
        }
        show_catalog();
        if (pref.catalog.filter.tag_scan_auto) scan_tags();
...
//      }
      ...
      function insert_thread_with_test(th, type, date_load){
        ...
        if (threads[name] && threads[name][8][0]>=th.time_bumped && threads[name][8][2]==th.nof_posts && threads[name][8][3]>=th.nof_files) {
          if (threads[name] && pref.catalog_footer_show_page && threads[name][24][2]!=th.page) insert_footer3(threads[name][24],threads[name][19],threads[name][8],name,null,th.page);
          return false;
        }
if (!pref.test_mode['0']) {
...
//        site2[th.domain].parse_funcs[type]['finisher'](th);
        insert_thread(th.pn, th.domain, th.page, date_load, th.key, (th.html_org)? th.html_org : th.pn.innerHTML, date, th.search_obj, url, true, th, type);
        return true;
      }
...
//      }
      function open_new_thread(url, name){
        ...
        var idx;
        var unread50 = (threads[name][19][0]>=0 && threads[name][8][2]-threads[name][19][2]<=50);
...
//        if (tgts) for (var name in tgts) if (threads[name] && threads[name][0].parentNode) threads[name][0].parentNode.removeChild(threads[name][0]); // this is required when the thread is moved to later. // THIS CAUSES A BLINK.
        ...
        var ref_height = (!pref.catalog_draw_on_demand)? 0
                                                       : (pref.catalog.embed && site.catalog)? brwsr.document_body.scrollTop + window.innerHeight*1.5
...
//var check_perf = ['catalog_refresh :', performance.now()];
        ...
        if (!('response' in value)) value.response = (read_type==='catalog_json')? JSON.parse(value.responseText)
                                                                                 : new DOMParser().parseFromString(value.responseText, 'text/html');
...
//      },
  ...
  if (pref.catalog.embed && site.catalog) catalog_obj.show_hide();
